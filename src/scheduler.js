...
const stopExecution = () => { console.log('Exiting...'); process.exit(0); };
...
const httpCaller = async (method, body, url, token = '') => {
  try {
    ...
    params = method === 'POST' ? { ...params, body: JSON.stringify(body) } : params;
...
const authenticate = async (mobile) => {
  try {
    ...
    const { txnId } = await httpCaller('POST', body, `${baseUrl}/v2/auth/generateMobileOTP`);
    ...
    const { token } = await httpCaller('POST', { txnId, otp: otpHashed }, `${baseUrl}/v2/auth/validateMobileOtp`);
    return token;
  } catch (error) {
    throw error;
  }
...
const getBeneficiaryIds = async (token) => {
  try {
    const { beneficiaries } = await httpCaller('GET', {}, `${baseUrl}/v2/appointment/beneficiaries`, token);
    ...
    let beneficiaryReferenceIds = beneficiaries.map((beneficiary) => {
      ...
      if (vaccination_status === 'Not Vaccinated') {
        if (2021 - parseInt(birth_year) < 45) {
          ...
          console.log('Beneficiary Reference Id: %d\n', beneficiary_reference_id);
...
const schedule = async (sessionScheduleDetails, token) => {
  try {
    const { center_id, session_id, beneficiaries, slots } = sessionScheduleDetails;
    const body = {
      dose: 1,
      center_id,
      session_id,
    ...
    const startTime = moment();
    while(momentTimeDiff(moment(), startTime, 'minutes') < 5) {
      const data = await httpCaller('POST', body, `${baseUrl}/v2/appointment/schedule`, token);
      if (data.appointmentId) {
        return data.appointmentId;
      } else {
        console.log(`Something wrong, received http status code: ${response.status}`);
      }
      await delay(100);
    }
  } catch (error) {
...
const looper = async (question) => {
  while (1) {
    const shouldProceed = await askQuestion(`${question}\nConfirm by typing \'Yes\' or \'No\'\n`);
...
const init = async () => {
  try {
    ...
    if (momentTimeDiff(startTime, moment()) > bufferTime) {
      ...
      console.log(`OTP will be requested ${bufferTime / 60} minutes before start time\
